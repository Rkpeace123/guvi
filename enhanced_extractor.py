...
class EnhancedIntelligenceExtractor:
    ...
    def _preprocess_message(self, message: str) -> str:
        ...
        message = message.replace(' at ', '@').replace(' dot ', '.')
        return message
    ...
    def _extract_phones_advanced(self, message: str) -> List[str]:
        ...
        for match in matches1:
            ...
            if len(cleaned) == 12 and cleaned.startswith('91'):  # +91 prefix
                phones.add(cleaned[2:])  # Store without country code
        ...
        # Pattern 4: Toll-free and landline numbers
        # 1800-XXX-XXXX, 022-XXXXXXXX
        pattern4 = r'\b(?:1800|022|011|080|044)[\s\-]?\d{3,4}[\s\-]?\d{4}\b'
        matches4 = re.findall(pattern4, message)
        ...
        for phone in phones:
            try:
                ...
                if phonenumbers.is_valid_number(phone_obj):
                    # Format consistently (remove country code for storage)
                    formatted = phonenumbers.format_number(phone_obj, phonenumbers.PhoneNumberFormat.NATIONAL)
    ...
    def _extract_upi_advanced(self, message: str) -> List[str]:
        ...
        upi_providers = [
            'paytm', 'phonepe', 'gpay', 'googlepay', 'upi', 
            'okaxis', 'ybl', 'ibl', 'axl', 'oksbi', 'okicici',
            'okhdfcbank', 'okbizaxis', 'fbl', 'pnb', 'boi'
        ]
        ...
        # Pattern 3: UPI with spaces (user @ paytm)
        pattern3 = r'\b[\w\.\-]+\s*@\s*[\w]+\b'
    ...
    def _extract_emails_advanced(self, message: str) -> List[str]:
        ...
        # Pattern 2: Obfuscated emails (already handled in preprocessing)
    ...
    def _extract_urls_advanced(self, message: str) -> List[str]:
        ...
        pattern3 = r'\b[a-zA-Z0-9-]+\.(com|net|org|in|co\.in|info|biz|xyz|tk|ml|ga|cf|gq)/[^\s]*'
        matches3 = re.findall(pattern3, message, re.IGNORECASE)
        for match in matches3:
            if isinstance(match, tuple):
                domain, ext = match
                urls.add(f'http://{domain}.{ext}')
            else:
                urls.add('http://' + match)
        
        # Pattern 4: Shortened URL services
        short_domains = ['bit.ly', 'tinyurl.com', 'goo.gl', 't.co', 'ow.ly', 'is.gd', 'buff.ly', 'cutt.ly', 'rb.gy']
        for domain in short_domains:
            pattern = rf'{domain}/[a-zA-Z0-9]+'
            matches = re.findall(pattern, message, re.IGNORECASE)
            urls.update(['http://' + m for m in matches])
        
        # Pattern 5: IP addresses as URLs
        pattern5 = r'\b(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?(?:/[^\s]*)?\b'
        matches5 = re.findall(pattern5, message)
...
if __name__ == "__main__":
    ...
    # Test cases
    test_messages = [
        "Call me at +91-9876543210 or 9876543210",
        "Send money to scammer@paytm or fraud@okaxis",
        "My email is scammer@fake-bank.com",
        "Account number: 1234567890123456",
        "Visit http://fake-sbi.com or www.phishing-site.com",
        "Transfer to 98765 43210 at user@phonepe",
        "Contact 1800-267-8000 for verification",
        "My UPI is scammer.fraud@ybl",
    ]
    
    print("="*60)
    print("ENHANCED INTELLIGENCE EXTRACTOR TEST")
    print("="*60)
